...
public abstract class ZylrAbstractContainerScreen<T extends AbstractContainerMenu> extends ChatScreen implements MenuAccess<T> {
    /** MODIFIED AbstractContainerScreen CODE */
    ...
    public ZylrAbstractContainerScreen(T menu, Inventory playerInventory, Component title, String initial) {
        super(initial);
        this.menu = menu;
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        //System.out.println("Mouse pre scale: " + mouseX + " : " + mouseY);
        RenderSystem.disableDepthTest();
        var _hudMenu = MinescapeAddons.getInstance().resizableClassic.getHudMenu();
        float scale = _hudMenu.scale;
        guiGraphics.pose().pushPose();
        guiGraphics.pose().translate((float)i, (float)j, 0.0F);
        _hudMenu.scaleAroundAnchor(guiGraphics, scale);
    ...
    protected void renderSlotHighlight(GuiGraphics guiGraphics, Slot slot, int mouseX, int mouseY, float partialTick) {
        if (slot.isHighlightable()) {
            if (Config.getFixedMode()) {
                float fixedScale = 2.5f;
                int slotSize = (int)(16 * fixedScale);
                int offsetX = (int)((slotSize - 16) / 2.0f);
                int offsetY = (int)((slotSize - 16) / 2.0f);
                guiGraphics.fillGradient(
                        RenderType.guiOverlay(),
                        slot.x - offsetX,
                        slot.y - offsetY,
                        slot.x - offsetX + slotSize,
                        slot.y - offsetY + slotSize,
    ...
    protected void renderSlot(GuiGraphics guiGraphics, Slot slot) {
        ...
        var hudMenuForSlot = MinescapeAddons.getInstance().resizableClassic.getHudMenu();
        guiGraphics.pose().pushPose();
        guiGraphics.pose().translate(0.0F, 0.0F, 100.0F);

        if (Config.getFixedMode()) {
            float fixedScale = 2.0f;
            // Translate to slot center, scale, translate back so it scales around the slot's position
            guiGraphics.pose().translate(i + 8, j + 8, 0.0F);
            guiGraphics.pose().scale(scale * fixedScale, scale * fixedScale, 1.0f);
            guiGraphics.pose().translate(-(i + 8), -(j + 8), 0.0F);
        } else {
            hudMenuForSlot.scaleAroundAnchor(guiGraphics, scale);
        }
    ...
    public void renderSlotContents(GuiGraphics guiGraphics, ItemStack itemstack, Slot slot, @Nullable String countString) {
     ...
     */
    /** Returns the hovered Widget (if any) from the active HUD, or null. */
    @Nullable
    private Widget getHoveredWidget() {
        for (IWidget iWidget : MinescapeAddons.getInstance().activeHud.widgets) {
            if (iWidget instanceof Widget widget && widget.isHovered()) {
                return widget;
            }
        }
        return null;
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        // Transform mouse into widget-space for button hit-testing (renderables are rendered scaled)
        double wx = hudMenu.toWidgetMouseX(mouseX);
        double wy = hudMenu.toWidgetMouseY(mouseY);
        if (super.mouseClicked(wx, wy, button)) {
            return true;
        } else {
            InputConstants.Key mouseKey = InputConstants.Type.MOUSE.getOrCreate(button);
            boolean flag = this.minecraft.options.keyPickItem.isActiveAndMatches(mouseKey);
            // findSlot uses isHovering which transforms internally — pass raw screen coords
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double dragX, double dragY) {
        // findSlot uses isHovering which transforms internally — use raw screen coords
        Slot slot = this.findSlot(mouseX, mouseY);
        ItemStack itemstack = this.menu.getCarried();
    ...
    public boolean mouseReleased(double mouseX, double mouseY, int button) {
        var hudMenu = MinescapeAddons.getInstance().resizableClassic.getHudMenu();
        // Transform for super (button widgets use unscaled positions, rendered scaled)
        super.mouseReleased(hudMenu.toWidgetMouseX(mouseX), hudMenu.toWidgetMouseY(mouseY), button);
        // findSlot uses isHovering which transforms internally — use raw screen coords
        Slot slot = this.findSlot(mouseX, mouseY);
        int i = this.leftPos;
        int j = this.topPos;
    ...
    protected boolean isHovering(int x, int y, int width, int height, double mouseX, double mouseY) {
        ...
        int j = this.topPos;
        var hudMenu = MinescapeAddons.getInstance().resizableClassic.getHudMenu();
        float scale = hudMenu.scale;
        int px = hudMenu.getAnchorX();
        int py = hudMenu.getAnchorY();
        // Inverse of scaleAroundParentAnchor: (screenMouse - pivot) / scale + pivot
        mouseX = (mouseX - px) / scale + px;
        mouseY = (mouseY - py) / scale + py;
        mouseX -= (double) i;
        mouseY -= (double) j;
        ...
        if (Config.getFixedMode()) {
            ...
            double halfW = (width / 2.0) * fixedScale;
            double halfH = (height / 2.0) * fixedScale;
            return mouseX >= centerX - halfW - 1
                    && mouseX < centerX + halfW + 1
                    && mouseY >= centerY - halfH - 1
                    && mouseY < centerY + halfH + 1;
        }
