...
@OnlyIn(Dist.CLIENT)
public abstract class ModContainerScreen<T extends Container> extends Screen implements IHasContainer<T> {
    ...
    protected int xSize = Minecraft.getInstance().getMainWindow().getScaledWidth();
    /** The Y size of the inventory window in pixels. */
    //protected int ySize = 166;
    protected int ySize = Minecraft.getInstance().getMainWindow().getScaledHeight();
...
//        System.out.println(this.guiTop);
        ...
        if (!ModConfiguration.CLIENT.rsInventory.get()) {
            if (Main.getInstance().getInv().getDefaultPostion())
                Main.getInstance().getInv().setDefaultPositions();
            ...
            renderRsInventory( p_render_1_, p_render_2_, p_render_3_);
        }
    ...
    private void renderRsInventory(int p_render_1_, int p_render_2_, float p_render_3_) {
        int[] inventory;

        switch(Main.getInstance().getStatsPanel().getCurrentScreen()) {
            case 1:
                inventory = Main.getInstance().getInv().INVENTORY_SLOT_ORDER;
                break;
            case 2:
                inventory = Main.getInstance().getInv().EQUIPEMENT_SLOT_ORDER;
                break;
            default:
                inventory = new int[0];
        }

        for (int slotOrder: inventory) {
            Slot slot = this.container.inventorySlots.get(slotOrder);
            ...
            if (this.isSlotSelected(slot, (double)p_render_1_, (double)p_render_2_) && slot.isEnabled()) {
                ...
                int k1 = slot.yPos;
                RenderSystem.colorMask(true, true, true, false);
                int slotColor = this.getSlotColor(slotOrder);
                this.fillGradient(j1, k1, j1 + 16, k1 + 16, slotColor, slotColor);
                RenderSystem.colorMask(true, true, true, true);
                RenderSystem.enableDepthTest();
                RenderSystem.popMatrix();
        ...
        if (!ModConfiguration.CLIENT.hotbar.get()) {
            for (int slotOrder : Main.getInstance().getInv().BUTTONS_ORDER) {
                Slot slot = this.container.inventorySlots.get(slotOrder);
        ...
        /*
        for(int i1 = 9; i1 < this.container.inventorySlots.size(); ++i1) {
            width += 16;
            Slot slot =  this.container.inventorySlots.get(i1);
                if (slot.isEnabled()) {
                    SlotsEditor.setX(slot, Main.getInstance().getInv().getInvXPostions().get(rsInvSlot) - 300);
                    SlotsEditor.setY(slot, Main.getInstance().getInv().getInvYPostions().get(rsInvSlot) - 300);
                    this.drawSlot(slot);
                    rsInvSlot++;
                }
            if (this.isSlotSelected(slot, (double)p_render_1_, (double)p_render_2_) && slot.isEnabled()) {
    ...
    /**
     *  MR_ZYLR posScale rescales for various Game Setting Gui Scales
     */
...
//        float posScale = (float)(Minecraft.getInstance().getMainWindow().getGuiScaleFactor() / 2);
        ...
        if (ModConfiguration.CLIENT.small.get())
            return mouseX >= (double)((x-width/6) - 1) && mouseX < (double)((x-width/6) + width*1.25 + 1)
                    && mouseY >= (double)(y - 1) && mouseY < (double)(y + height);
        if (ModConfiguration.CLIENT.rsInventory.get())
            return mouseX >= (double)((x-width/4) - 1) && mouseX < (double)((x-width/4) + width*1.5 + 2)
                    && mouseY >= (double)((y-height/6) - 1) && mouseY < (double)((y-height/6) + height*1.25 + 1);
    ...
    public boolean keyPressed(int p_keyPressed_1_, int p_keyPressed_2_, int p_keyPressed_3_) {
        if (super.keyPressed(p_keyPressed_1_, p_keyPressed_2_, p_keyPressed_3_)) {
            ...
            if (this.itemStackMoved(p_keyPressed_1_, p_keyPressed_2_))
                return true; // Forge MC-146650: Needs to return true when the key is handled.
            if (this.hoveredSlot != null && this.hoveredSlot.getHasStack()) {
                if (this.minecraft.gameSettings.keyBindPickBlock.isActiveAndMatches(mouseKey)) {
                    ...
                    // Stop drop if protected slot turned on
                    if (this.hoveredSlot.getSlotIndex() == 1 && ModConfiguration.CLIENT.protectedSlot.get())
                        return true; // Forge MC-146650: Needs to return true when the key is handled.
    ...
    /**
     * Determines whether or not an ItemStack can be moved from the hotbar to a hovered slot.
     */
    ...
    public void updateSize() {
        if (ModConfiguration.CLIENT.rsInventory.get()) {
            xSize = Minecraft.getInstance().getMainWindow().getScaledWidth();
            ySize = Minecraft.getInstance().getMainWindow().getScaledHeight();
            this.guiLeft = (this.width - this.xSize) / 2;
            this.guiTop = (this.height - this.ySize) / 2;
        }
    }
