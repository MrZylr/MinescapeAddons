...
public final class SearchResultsScreen extends Screen implements HudTabRestoringScreen {
	...
	private static final int SLOT_START_X = 29;
	...
	private static final int HIDDEN_ROW_SLOT_INDEX = 8;
	...
	public SearchResultsScreen(ChestMenu menu, Inventory inventory, Component title, boolean focusInventoryOnOpen) {
		super(title);
		this.menu = menu;
	...
	public boolean mouseScrolled(double mouseX, double mouseY, double horizontalAmount, double verticalAmount) {
		if (verticalAmount != 0.0 && this.clickNamedItem(verticalAmount > 0.0 ? 8 : 53)) {
			return true;
		}
	...
	private void drawChestItems(GuiGraphicsExtractor graphics) {
		...
		for (int slotId = 0; slotId < chestSlotCount; slotId++) {
			if (!this.isVisibleChestSlot(slotId)) {
				continue;
			}
			Slot slot = this.menu.slots.get(slotId);
	...
	private boolean clickNamedItem(int slotId) {//8, 53
		if (slotId == 8 || slotId == 53) {
	...
	private boolean isVisibleChestSlot(int slotId) {
		return slotId >= 0 && slotId < this.chestSlotCount() && (slotId % CHEST_ROW_SLOT_COUNT) < SLOT_COLS;
	}
