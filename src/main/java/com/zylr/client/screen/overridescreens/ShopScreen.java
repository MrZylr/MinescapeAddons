...
public final class ShopScreen extends Screen implements HudTabRestoringScreen {
	...
	private static final int CHEST_GRID_X = 22;
	private static final int CHEST_GRID_Y = 44;
	private static final int CHEST_GRID_W = 318;
	private static final int CHEST_GRID_H = 192;
	private static final int CHEST_SLOT_COLS = 9;
	private static final int CHEST_SLOT_GAP = 2;
	...
	private int leftPos;
	private int topPos;
	private int chestGridStartX;
	private int chestGridStartY;
	private int slotStep;
	private int itemDrawSize;
	private HudTab previousHudTab;

	public ShopScreen(ChestMenu menu, Inventory inventory, Component title, boolean focusInventoryOnOpen) {
		super(title);
		this.menu = menu;
	...
	private void layoutSlots() {
		int visibleChestSlots = this.visibleChestSlotCount();
		int chestRows = Math.max(1, (visibleChestSlots + CHEST_SLOT_COLS - 1) / CHEST_SLOT_COLS);
		this.slotStep = Math.min(
			CHEST_GRID_W / CHEST_SLOT_COLS,
			CHEST_GRID_H / Math.max(1, chestRows)
		);
		this.itemDrawSize = Math.max(16, this.slotStep - CHEST_SLOT_GAP * 2);
		int gridWidth = CHEST_SLOT_COLS * this.slotStep;
		int gridHeight = chestRows * this.slotStep;
		this.chestGridStartX = CHEST_GRID_X + (CHEST_GRID_W - gridWidth) / 2;
		this.chestGridStartY = CHEST_GRID_Y + (CHEST_GRID_H - gridHeight) / 2;
		...
		for (int slotId = 0; slotId < this.menu.slots.size(); slotId++) {
			...
			if (slotId < visibleChestSlots) {
				int col = slotId % CHEST_SLOT_COLS;
				int row = slotId / CHEST_SLOT_COLS;
				((SlotAccessor) slot).minescapeaddon$setX(this.chestGridStartX + col * this.slotStep);
				((SlotAccessor) slot).minescapeaddon$setY(this.chestGridStartY + row * this.slotStep);
			} else {
	...
	private void drawChestItems(GuiGraphicsExtractor graphics) {
		for (int slotId = 0; slotId < this.visibleChestSlotCount(); slotId++) {
			Slot slot = this.menu.slots.get(slotId);
			...
			this.drawScaledItem(graphics, slot.getItem(), drawX, drawY, this.itemDrawSize);
		}
	}
	...
	private void drawScaledItem(GuiGraphicsExtractor graphics, net.minecraft.world.item.ItemStack stack, int x, int y, int drawSize) {
		...
		StackSizeOverlay.renderLightweightStockAndStackOrCount(graphics, this.font, stack, centeredX, centeredY, drawSize);
	}
	...
	private void drawTooltips(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
		...
		if (contains(relativeX, relativeY, HEADER_TUTORIAL_X, HEADER_TUTORIAL_Y, HEADER_TUTORIAL_W, HEADER_TUTORIAL_H)) {
			Slot tutorialSlot = this.tutorialSlot();
	...
	private Slot findSlot(int mouseX, int mouseY) {
		for (int slotId = 0; slotId < this.visibleChestSlotCount(); slotId++) {
			Slot slot = this.menu.slots.get(slotId);
			int x = this.leftPos + slot.x - Math.round((this.itemDrawSize - 16) / 2.0F);
			int y = this.topPos + slot.y - Math.round((this.itemDrawSize - 16) / 2.0F);
			if (contains(mouseX, mouseY, x, y, this.itemDrawSize, this.itemDrawSize)) {
				return slot;
			}
		}
		return null;
	...
	private int chestSlotCount() {
		return this.menu.getRowCount() * CHEST_SLOT_COLS;
	}

	private int visibleChestSlotCount() {
		return Math.max(0, this.chestSlotCount() - 1);
	}

	private Slot tutorialSlot() {
		int tutorialSlotId = this.chestSlotCount() - 1;
		if (tutorialSlotId < 0 || tutorialSlotId >= this.menu.slots.size()) {
			return null;
		}
		return this.menu.slots.get(tutorialSlotId);
	}
